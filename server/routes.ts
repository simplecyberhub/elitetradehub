...
export async function registerRoutes(app: Express): Promise<Server> {
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (verificationStatus === "verified") {
          await storageInstance.updateUser(existingDoc.userId, {
            kycStatus: "verified",
          });
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const pendingKyc = allDocuments.filter(
  ...
  app.patch(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const updatedAsset = await storageInstance.updateAsset(
          assetId,
          updates,
        );
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        if (typeof lockPeriodDays === "string") {
          const durationMatch = lockPeriodDays.match(
        ...
        const newPlan =
          await storageInstance.createInvestmentPlan(validatedData);
  ...
  app.patch(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const updates = req.body;
        const updatedPlan = await storageInstance.updateInvestmentPlan(
          planId,
          updates,
        );
      ...
      try {
        ...
        const investmentsWithDetails = await Promise.all(
          allInvestments.map(async (investment) => {
            ...
            const plan = await storageInstance.getInvestmentPlan(
              investment.planId,
            );
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const csvRows = data.map((item) => {
          ...
          return Object.values(itemWithoutPassword)
            .map((value) =>
              typeof value === "string"
                ? `"${value.replace(/"/g, '""')}"`
                : value,
  ...
  app.post(
    ...
    asyncHandler(async (req: Request, res: Response) => {
      ...
      const existingUser = await storageInstance.getUserByUsername(
      ...
      const existingEmail = await storageInstance.getUserByEmail(
        sanitizedData.email,
      );
      if (existingEmail) {
  ...
  app.get("/api/traders", async (req: Request, res: Response) => {
    try {
      ...
      const tradersWithDetails = await Promise.all(
  ...
  app.post("/api/copy-trading", async (req: Request, res: Response) => {
    try {
      ...
      const existingRelationships =
        await storageInstance.getCopyRelationshipsByFollowerId(
          validatedData.followerId,
        );
      const existingRelationship = existingRelationships.find(
        (r) => r.traderId === validatedData.traderId,
      );

      if (existingRelationship) {
        return res
          .status(400)
          .json({ message: "Already following this trader" });
      }

      const newRelationship =
        await storageInstance.createCopyRelationship(validatedData);
  ...
  app.get(
    "/api/user/:userId/copy-trading",
    async (req: Request, res: Response) => {
      try {
        const userId = parseInt(req.params.userId);
        const relationships =
          await storageInstance.getCopyRelationshipsByFollowerId(userId);

        // Get trader details for each relationship
        const relationshipsWithDetails = await Promise.all(
          relationships.map(async (relationship) => {
            const trader = await storageInstance.getTrader(
  ...
  app.patch("/api/copy-trading/:id", async (req: Request, res: Response) => {
    try {
      ...
      const relationship =
        await storageInstance.getCopyRelationship(relationshipId);
      ...
      const updatedRelationship = await storageInstance.updateCopyRelationship(
        relationshipId,
        validatedData,
      );
  ...
  app.delete("/api/copy-trading/:id", async (req: Request, res: Response) => {
    try {
      ...
      const result =
        await storageInstance.deleteCopyRelationship(relationshipId);
  ...
  app.post("/api/trades", async (req: Request, res: Response) => {
    try {
      ...
      if (
        !validatedData.copiedFromTradeId &&
  ...
  app.get("/api/user/:userId/trades", async (req: Request, res: Response) => {
    try {
      ...
      const tradesWithAssets = await Promise.all(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const maxAmount = plan.maxAmount
          ? parseFloat(plan.maxAmount.toString())
          : Infinity;

        if (investmentAmount < minAmount) {
          return res
            .status(400)
            .json({ message: `Minimum investment amount is ${minAmount}` });
        }

        if (maxAmount !== 0 && investmentAmount > maxAmount) {
          return res
            .status(400)
            .json({ message: `Maximum investment amount is ${maxAmount}` });
        }

        // Create the investment
        const newInvestment =
          await storageInstance.createInvestment(validatedData);
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const investments =
          await storageInstance.getInvestmentsByUserId(userId);
  ...
  app.post("/api/transactions", async (req: Request, res: Response) => {
    try {
      ...
      const newTransaction =
        await storageInstance.createTransaction(validatedData);
  ...
  app.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const transactions =
          await storageInstance.getTransactionsByUserId(userId);
  ...
  app.post("/api/kyc-documents", async (req: Request, res: Response) => {
    try {
      ...
      const newDocument =
        await storageInstance.createKycDocument(validatedData);
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const watchlistItems =
          await storageInstance.getWatchlistItemsByUserId(userId);
